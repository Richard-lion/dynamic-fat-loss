...
export async function GET(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      if (log.weight !== null) {
        sum += log.weight;
        weightData.push({ date: dates[i], weight: log.weight, movingAvg: Math.round((sum / (i + 1)) * 10) / 10 });
      }
    }

    const recentLogs = dates.slice(-7).map(d => state.dailyLogs[d]);
    const macroTotal = recentLogs.reduce(
      (acc, log) => ({ carbs: acc.carbs + log.totalCarbs, protein: acc.protein + log.totalProtein, fat: acc.fat + log.totalFat, calories: acc.calories + log.calories }),
